...
const checkUserJoinedChannels = async (userId) => {
  const channels = ['t.me/fahimhussine', 't.me/fahimhussine'];
  let allJoined = true;
...
const sendChannelsRequiredMessage = async (chatId) => {
  return bot.sendMessage(chatId,
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '📢 Channel 1', url: 'https://whatsapp.com/channel/0029Vb8z40LLNSaBTaPMKU3E' }],
          [{ text: '📢 Channel 2', url: 'https://t.me/xml_edit_buy_fahim' }],
          [{ text: '👥 Group', url: 'https://chat.whatsapp.com/HsC4RawxuVaAFFmSKtzeKC' }],
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    ...
    {
      caption: `🪀 *𝙏𝙝𝙚 Fahim 𝑴𝑫💀*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: "👑 Owner", url: "t.me/fahimhussine" }]
        ]
      }
    }
  );
  ...
  if (!allJoined) {
    return bot.sendMessage(chatId,
      `🚨 *You must join our official channels before pairing.*`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '📢 Channel 1', url: 'https://chat.whatsapp.com/HsC4RawxuVaAFFmSKtzeKC' }],
            [{ text: '📢 Channel 2', url: 't.me/fahimhussine' }],
            [{ text: '👥 Group', url: 'https://t.me/fahim_md_bot1' }],
            [{ text: '✅ I have joined', callback_data: 'check_join' }]
          ]
        }
      }
...
bot.onText(/\/unpair(?:\s+(.+))?/, async (msg, match) => {
  ...
  try {
    if (!input) {
      return bot.sendMessage(chatId, 'Example: /unpair 8801xxxxxxxxx', { parse_mode: 'Markdown' });
    }
    if (/[a-z]/i.test(input)) {
      return bot.sendMessage(chatId, 'Letters not allowed. Use: /unpair 8801xxxxxxxxx', { parse_mode: 'Markdown' });
    }
    if (!/^\d{7,15}$/.test(input)) {
      return bot.sendMessage(chatId, 'Invalid format. Use: /unpair 8801xxxxxxxxx', { parse_mode: 'Markdown' });
    }
